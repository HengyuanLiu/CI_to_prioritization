...
if not os.path.exists(save_path_gamma):
    os.makedirs(save_path_gamma)

save_path = save_path_gamma + '/'
...
for learn_rate in learn_rate_list:
    for alpha in alpha_list:
        ...
        plt.ion()
        plt.plot(x, y_2_pred, 'b--', label='gamma='+str(0.2))
        plt.plot(x, y_5_pred, 'b-', label='gamma='+str(0.5))
        plt.plot(x, y_8_pred, 'b-.', label='gamma='+str(0.8))
        plt.plot(x, y_max_pred, 'r', label='NAPFD_max')
        plt.plot(x, y_min_pred, 'g', label='NAPFD_min')
        plt.legend()
        plt.xlabel('Stage')
        plt.ylabel('NAPFD')
        plt.title('alpha='+str(alpha)+' lr='+str(learn_rate))
        save_name = str(alpha) + '_' + str(learn_rate) + '.jpg'
